...
class SharedReplayBuffer(object):
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        for indices in sampler:
            ...
            # 此时share_obs_batch列表中有mini_batch_size个长度为data_chunk_length的chunk
            # rnn_states_batch列表中有mini_batch_size个hidden_state, 每个hidden_state的
            # shape为(recurrent_N, hidden_size), 并且实际是对应的chunk的初始hidden_state
...
class RNN_Cognition_Buffer(SharedReplayBuffer):
    def __init__(self, args, num_agents, num_enemies, obs_space, cent_obs_space, act_space):
        super().__init__(args, num_agents, obs_space, cent_obs_space, act_space)
        self.latent_dim = args.latent_dim
        self.rnn_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, self.recurrent_N, (num_agents + num_enemies) * self.latent_dim),
            dtype=np.float32)
        self.rnn_states_critic = np.zeros_like(self.rnn_states)

class subtask_Buffer(SharedReplayBuffer):
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        super().__init__(args, num_agents, obs_space, cent_obs_space, act_space)
        self.latent_dim = args.latent_dim
        self.num_subtask = args.num_subtask
        self.rnn_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, self.recurrent_N, self.latent_dim),
            dtype=np.float32)
        self.rnn_states_critic = np.zeros_like(self.rnn_states)

        self.subtasks = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, self.num_subtask), dtype=np.float32)
        
        if args.subtask_to_rnn:
            old_share_obs_shape = self.share_obs.shape
            new_share_obs_shape = list(old_share_obs_shape)
            new_share_obs_shape[-1] += self.num_subtask
            self.share_obs = np.zeros(new_share_obs_shape, dtype=np.float32)

            old_obs_shape = self.obs.shape
            new_obs_shape = list(old_obs_shape)
            new_obs_shape[-1] += self.num_subtask
            self.obs = np.zeros(new_obs_shape, dtype=np.float32)

        

# class SharedReplayBuffer4Traj(SharedReplayBuffer):
#     def __init__(self, args, num_agents, num_enemies, obs_space, cent_obs_space, act_space):
#         super().__init__(args, num_agents, obs_space, cent_obs_space, act_space)
#         self.latent_dim = args.latent_dim
#         self.entity_observation = np.zeros(
#             (self.episode_length + 1, self.n_rollout_threads, num_agents, num_agents + num_enemies, 16),
#             dtype=np.float32
#         )
#         self.entity_state = np.zeros(
#             (self.episode_length + 1, self.n_rollout_threads, num_agents, num_agents + num_enemies, 20),
#             dtype=np.float32
#         )
#         self.cognition = np.zeros(
#             (self.episode_length + 1, self.n_rollout_threads, num_agents, num_agents + num_enemies, self.latent_dim),
#             dtype=np.float32
#         )
