...
class StarCraft2UnifiedEnv(MultiAgentEnv):
    ...
    def __init__(
        ...
        print(f"share_obs_space: {self.share_observation_space}")
    ...
    def get_obs(self):
        ...
        # transfer
        agents_obs = ft.translate_local_obs(agents_obs, self.args.use_map_ohid)
        return agents_obs
    ...
    def get_state_agent(self, agent_id):
        ...
        if (self.use_mustalive and unit.health > 0) or (not self.use_mustalive):  # otherwise dead, return all zeros
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if e_unit.health > 0:  # visible and alive
                    ...
                    if unit.health > 0:
                        if dist < sight_range:
                            enemy_feats[e_id, 0] = 1 # visible
                        enemy_feats[e_id, 1] = dist / sight_range  # distance
                        enemy_feats[e_id, 2] = (e_x - x) / sight_range  # relative X
                        enemy_feats[e_id, 3] = (e_y - y) / sight_range  # relative Y
                        enemy_feats[e_id, 4] = avail_actions[self.n_actions_no_attack + e_id]  # available
        ...
        state = np.concatenate((own_feats.flatten(),
                                ally_feats.flatten(), 
                                enemy_feats.flatten(),
                                move_feats.flatten()))
        if self.args.entity_feature:
            state = ft.single_translate_global(self.map_name, state, self.obs_all_health, self.add_center_xy, self.args.use_map_ohid)
        # Agent id features
    ...
    def get_obs_size(self):
        ...
        # all_feats = 16*(n_allies+n_enemies+1)+move_feats
        all_feats = (16 + 23)*(n_allies+n_enemies+1)+move_feats if self.args.use_map_ohid else 16*(n_allies+n_enemies+1)+move_feats
    ...
    def get_state_size(self):
        ...
        if self.use_state_agent:
            ...
            if self.args.entity_feature:
                n_enemy_feats = n_ally_feats = own_feats = 20 + 23 if self.args.use_map_ohid else 20
